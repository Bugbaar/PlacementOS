"""CLI entrypoint for the eligibility & analytics demo."""
...
def main():
    students = load_csv_data(ROOT / "data" / "students.csv")
    placement_drives = load_csv_data(ROOT / "data" / "placement_drives.csv")

    results = []
    for student in students:
        for drive in placement_drives:
            results.append(check_eligibility(student, drive))

    summary = generate_summary(results)
    top_missing_skills = find_top_missing_skills(results)
    branch_analytics = generate_branch_analytics(results, students)
