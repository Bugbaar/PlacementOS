"""Aggregate analytics over eligibility evaluation results."""
...
def find_top_missing_skills(results):
    """
    Rank most commonly missing skills across evaluations.

    Skills are expected to already be normalized to a stable form
    (lowercase) by the eligibility engine.
    """
    missing_skills = []

    for result in results:
        for skill in result["missing_skills"]:
            missing_skills.append(str(skill).strip().lower())

    return Counter(missing_skills).most_common()
...
def generate_branch_analytics(results, students):
    ...
    for branch, values in branch_results.items():
        ...
        rate = round((eligible / total) * 100, 2) if total else 0
