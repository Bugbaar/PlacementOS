...
exports.chatWithAssistant = exports.getStudentContext = void 0;
...
const getStudentContext = async (studentId) => {
    ...
    const opportunities = await Opportunity_1.default.find({ status: 'active' });
    const applications = await Application_1.default.find({ studentId }).populate('opportunityId', 'title company');
    // Generate recommendations and match scores deterministically
    const recommendations = opportunities.map((opp) => {
        const match = (0, matchingService_1.calculateMatchScore)(student, opp);
        return {
            title: opp.title,
            company: opp.company,
...
exports.getStudentContext = getStudentContext;
...
const chatWithAssistant = async (studentId, userMessage, conversationHistory) => {
    const context = await (0, exports.getStudentContext)(studentId);
    ...
    try {
        const aiResponse = await (0, groqService_1.generateAIResponse)(systemPrompt, userMessage, recentHistory);
        return aiResponse;
    }
    catch (error) {
