...
export function runEligibilityEngine(
  ...
  const logs: string[] = [
    `[SYSTEM] Ingested ${students.length} student records`,
    `[SYSTEM] Criteria — ${criteria.companyName} / ${criteria.driveName}`,
    `[SYSTEM] CGPA ≥ ${criteria.minCgpa} · backlogs ≤ ${criteria.maxActiveBacklogs} · skills (${criteria.skillMatchMode}): ${
  ...
  const results = students.map((student) => {
    ...
    const detail = evaluation.eligible
      ? `skills: ${evaluation.matchedSkills.join(', ') || 'n/a'}`
      : evaluation.reasons.join(' · ');
    logs.push(`[${stamp}] ${student.rollNumber} ${student.name}… ${tag} · ${detail}`);
    return evaluation;
  });
  ...
  logs.push(`[COMPLETE] ${shortlisted.length}/${results.length} shortlisted in ${elapsedMs}ms`);
